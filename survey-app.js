...
//         form.method = "POST";
//         form.action =
//           "https://script.google.com/macros/s/AKfycbyhyZW3KhZFc4pcknES0hmTW9vqkW_Ts5SWiiGVzvb8hpYu2IimDOWuqkmrXJpQborr/exec"; //overwrite here url and then check
...
//   }
// });
// full updated script with "Other" behavior changed as requested
...
//           // ---- MODIFIED: remove inline "Other" textbox logic and instead manage temporary question insertion/removal ----
//           // If the user deselects "Other" (or the set of selected options no longer includes "Other"),
//           // remove the temporary "other" question inserted earlier (if any).
//           const otherQuestionId = q.name + "_other_question"; // id used for temporary inserted question
//           const hasOtherSelected =
//             (q.type === "checkbox" &&
//               allAnswers[q.name] &&
//               Array.isArray(allAnswers[q.name]) &&
//               allAnswers[q.name].includes("Other")) ||
//             (q.type === "radio" && allAnswers[q.name] === "Other");

//           if (!hasOtherSelected) {
//             const idx = questionOrder.findIndex(
//               (item) => item.id === otherQuestionId
//             );
//             if (idx !== -1) {
//               questionOrder.splice(idx, 1);
//               // If removed question is before or equal to currentStep, adjust currentStep to keep the UI stable
...
//         // NOTE: Removed the inline 'Other' text input insertion here.
//         // The inline insertion block was deleted so that "Other" doesn't show small textboxes immediately.
//         // Instead, a new temporary question is inserted when the user clicks Next (see nextBtn handler).
//       });
//     } else if (q.type === "text" || q.type === "email") {
//       const input = document.createElement("input");
//       input.type = q.type;
...
//       // If 'Other' is selected, validate the other field
//       // NOTE: Previously inline 'other' input was validated here; now 'Other' leads to a separate question,
//       // so this block will only run if an inline other input exists (which it no longer will).
//       const values = Array.from(checked).map((el) => el.value);
//       if (values.includes("Other")) {
//         const otherFieldId = q.name + "_other";
//         const otherInput = document.querySelector(`[name='${otherFieldId}']`);
//         if (otherInput && otherInput.value.trim() === "") {
//           // If a separate "other" text field exists inline (not the new separate question), validate it.
//           // But normally we will redirect user to a new question to collect this value instead.
//           showValidationError(q, 'Please specify your "Other" option');
//           return false;
//         }
//       }
...
//       // If 'Other' is selected, validate the other field
//       // As above, inline 'Other' input is no longer shown; the dedicated question will collect the value.
//       if (checked.value === "Other") {
//         const otherFieldId = q.name + "_other";
//         const otherInput = document.querySelector(`[name='${otherFieldId}']`);
//         if (otherInput && otherInput.value.trim() === "") {
...
//       // ---- MODIFIED: If current question has "Other" selected, insert a new text question next ----
//       const q = questionOrder[currentStep];
//       if (q) {
...
//         if (isOtherSelected) {
//           // Prepare an id and name for the temporary "other" question
//           const otherQuestionId = `${q.name}_other_question`;
//           const otherFieldName = `${q.name}_other`;

//           // Avoid inserting duplicate temporary question
//           const alreadyInserted = questionOrder.some(
//             (item) => item.id === otherQuestionId
//           );

//           if (!alreadyInserted) {
//             // Create temporary question object that matches the format used elsewhere
//             const specifyOtherQuestion = {
//               id: otherQuestionId,
//               text: "Please specify other option(s)",
//               type: "text",
//               name: otherFieldName,
//               placeholder: "Please specify",
//               required: true,
//               // Not conditional: we insert it only when needed, and remove when not needed
//             };

//             // Insert the temporary question right after the current question
//             questionOrder.splice(currentStep + 1, 0, specifyOtherQuestion);
//           }

//           // Move to the temporary "other" question
//           currentStep++;
//           showQuestion(currentStep);
//           return;
//         }
  ...
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      data.forEach((loc) => {
        ...
        div.style.cssText =
          "padding:10px;cursor:pointer;border-bottom:1px solid #eee;transition:background .2s;";

        let name = loc.display_name;
        if (loc.address) {
          const p = [];
          if (loc.address.city || loc.address.town || loc.address.village)
            p.push(loc.address.city || loc.address.town || loc.address.village);
          if (loc.address.state) p.push(loc.address.state);
          if (loc.address.country) p.push(loc.address.country);
          if (p.length) name = p.join(", ");
        }
        div.textContent = name;

        div.onmouseenter = () => (div.style.backgroundColor = "#f5f5f5");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /* ----------  INITIALISE ---------- */
  function initializeSurvey() {
    questionOrder = [...basicInfoQuestions, rootQuestion];
    showQuestion(currentStep);
  }

  /* ----------  BRANCHING ---------- */
  function handleBranchSelection() {
    const root = allAnswers[rootQuestion.name];
    questionOrder = questionOrder.slice(0, basicInfoQuestions.length + 1);

    if (root === "Yes") {
      branchType = "owner";
      questionOrder.push(...petOwnerQuestions);
    } else if (root === "No, but planning to get one soon") {
      branchType = "future";
      questionOrder.push(...futureOwnerQuestions);
    } else if (root === "No, and not planning to get one") {
      branchType = "provider";
      // Do NOT push serviceProviderQuestions here
    }

    // SERVICE QUESTIONS FOR ALL USERS
    questionOrder.push(...serviceProviderQuestions);
  ...
  function showQuestion(step) {
    ...
    /* ---- COMBINED (personal info) ---- */
    if (q.type === "combined") {
      const wrap = document.createElement("div");
      wrap.className = "combined-fields";
      q.fields.forEach((f) => {
        const fc = document.createElement("div");
        fc.className = "field-container";
        ...
        if (f.type === "location") {
          ...
          input.value = allAnswers[f.name] || "";
          input.autocomplete = "off";

          const ac = document.createElement("div");
          ac.className = "autocomplete-container";
          ac.style.position = "relative";
          const sugg = document.createElement("div");
          sugg.className = "location-suggestions";
          sugg.style.cssText =
            "position:absolute;top:100%;left:0;right:0;background:white;border:1px solid #ddd;border-top:none;max-height:200px;overflow-y:auto;z-index:1000;display:none;";
          ...
          input.addEventListener("input", () => {
            ...
            timer = setTimeout(
              () => fetchLocationSuggestions(val, sugg, input),
              300
            );
          });
          document.addEventListener("click", (e) => {
            if (!ac.contains(e.target)) sugg.style.display = "none";
          });
          ...
          input.placeholder = f.placeholder || "";
          input.value = allAnswers[f.name] || "";
          if (f.type === "email" && f.validation) {
            input.pattern = f.validation.toString().slice(1, -1);
            input.title = "Valid email required";
          }
      ...
      q.options.forEach((opt, i) => {
        ...
        if (
          q.type === "checkbox" &&
          Array.isArray(allAnswers[q.name]) &&
          allAnswers[q.name].includes(opt.value)
        )
          inp.checked = true;
        if (q.type === "radio" && allAnswers[q.name] === opt.value)
          inp.checked = true;
        ...
        /* ---- CHANGE HANDLER (root + Other removal) ---- */
        inp.addEventListener("change", () => {
          ...
          /* ---- remove stale Other-question if Other is unchecked ---- */
          const otherQId = `${q.name}_other_question`;
          const hasOther =
            (q.type === "checkbox" &&
              Array.isArray(allAnswers[q.name]) &&
              allAnswers[q.name].includes("Other")) ||
          ...
          if (!hasOther) {
            const idx = questionOrder.findIndex((it) => it.id === otherQId);
            if (idx !== -1) {
              questionOrder.splice(idx, 1);
              if (idx <= currentStep)
                currentStep = Math.max(0, currentStep - 1);
      ...
      inp.value = allAnswers[q.name] || "";
      if (q.name === "emailConfirm" || q.name === "emailConfirmOptional")
        inp.value = allAnswers.emailPrimary || "";
      if (q.type === "email" && q.validation) {
        inp.pattern = q.validation.toString().slice(1, -1);
        inp.title = "Valid email required";
      }
      questionContainer.appendChild(inp);
    } else if (q.type === "textarea") {
      const ta = document.createElement("textarea");
      ta.name = q.name;
      ta.placeholder = q.placeholder || "";
      ta.value = allAnswers[q.name] || "";
  ...
  /* ----------  PROGRESS BAR ---------- */
  function updateProgressBar() {
    const prog = ((currentStep + 1) / questionOrder.length) * 100;
    progressFill.style.width = `${prog}%`;
    document.getElementById("current-step").textContent = `Step ${
      currentStep + 1
    }`;
    document.getElementById("total-steps").textContent = questionOrder.length;
  }
  ...
  function saveCurrentAnswer() {
    ...
    if (q.type === "combined") {
      q.fields.forEach((f) => {
        ...
        allAnswers[f.name] = el ? el.value.trim() : "";
      });
    } else if (q.type === "checkbox") {
      allAnswers[q.name] = Array.from(
        document.querySelectorAll(`input[name='${q.name}']:checked`)
      ...
      allAnswers[q.name] = el ? el.value.trim() : "";
    }
  }
  ...
  function validateCurrentQuestion() {
    ...
    if (q.type === "combined") {
      for (const f of q.fields) {
        ...
        const el = document.querySelector(`[name='${f.name}']`);
        if (!el || !el.value.trim()) {
          showValidationError(q, `Please fill ${f.text}`);
          el?.focus();
          return false;
        }
        if (
          f.type === "email" &&
          f.validation &&
          !f.validation.test(el.value.trim())
        ) {
          showValidationError(q, "Valid email required");
          el.focus();
          return false;
        }
        if (f.type === "location" && el.value.trim().length < 2) {
          showValidationError(q, "Enter a valid location");
          el.focus();
          return false;
        }
      }
      ...
      if (
        !document.querySelectorAll(`input[name='${q.name}']:checked`).length
      ) {
        showValidationError(q, "Select at least one option");
        return false;
      }
    } else if (q.type === "radio") {
      if (!document.querySelector(`input[name='${q.name}']:checked`)) {
        showValidationError(q, "Select an option");
        return false;
      }
    } else if (q.type === "email") {
      const el = document.querySelector(`[name='${q.name}']`);
      if (!el || !el.value.trim()) {
        showValidationError(q, "Email required");
        return false;
      }
      if (q.validation && !q.validation.test(el.value.trim())) {
        showValidationError(q, "Valid email required");
        return false;
      }
    } else {
  ...
  function showValidationError(q, msg) {
    clearValidationError();
    ...
    err.textContent = msg;
    questionContainer.appendChild(err);
  }
  function clearValidationError() {
    const e = questionContainer.querySelector(".validation-error");
    if (e) e.remove();
  }

  /* ----------  NEXT BUTTON – INSERT OTHER QUESTION ---------- */
  nextBtn?.addEventListener("click", function () {
    ...
    /* ---- INSERT FOLLOW-UP IF “Other” IS SELECTED ---- */
    const isOtherSelected =
      (q.type === "radio" && allAnswers[q.name] === "Other") ||
      (q.type === "checkbox" &&
        Array.isArray(allAnswers[q.name]) &&
    ...
    if (isOtherSelected) {
      ...
      const already = questionOrder.some((it) => it.id === otherQId);
      if (!already) {
        // const otherQ = {
        //   id: otherQId,
        //   text: "Please specify other option(s)",
        ...
        // };
        const textMap = {
          q_petTypes: "Please specify other pet type(s)",
          q_biggestChallenge: "Please specify your biggest challenge",
          q_preferredType: "Please specify other pet type(s)",
          q_mainConcern: "Please specify your main concern",
          q_serviceTypes: "Please specify other service type(s)",
        };

        const otherQ = {
          id: otherQId,
          text: textMap[q.id] || "Please specify other option(s)",
          type: "text",
          name: otherFieldName,
          placeholder: "Enter details…",
          required: true,
        };
        questionOrder.splice(currentStep + 1, 0, otherQ); // insert right after current
      }
      currentStep++; // move to the new question
      showQuestion(currentStep);
      return;
    }
    ...
    if (currentStep < questionOrder.length - 1) {
      ...
      form.method = "POST";
      form.action =
        "https://script.google.com/macros/s/AKfycbzXcPnH8_K-m4tVlfbygwCFudOI3wtjCbhylNbu5qGAFM_7SQc3Oiku2SDPoHYrg0OrUw/exec";
      form.style.display = "none";
